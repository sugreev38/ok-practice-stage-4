...
export default function PurchasedTestsPage() {
  ...
  useEffect(() => {
    async function fetchPurchasedTests() {
      try {
        const result = await getPurchasedTests(purchasedTests)

        if (result.success) {
          setPurchasedTests(result.purchasedTests)
        } else {
          toast({
            title: "Error",
            description: result.error || "Failed to fetch purchased tests",
            variant: "destructive",
          })
        }
      } catch (error) {
        ...
        toast({
          title: "Error",
          description: "Failed to fetch purchased tests",
          variant: "destructive",
        })
      } finally {
        setIsLoading(false)
