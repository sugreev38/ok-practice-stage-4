...
export default function AttemptTestPage({ params }: AttemptTestPageProps) {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (response.status === 403) {
              throw new Error("Forbidden: You do not have access to purchased tests");
            }
  ...
  }, [isAuthenticated, isLoading, user, router, testId, toast]);
    ...
    const fetchTestResult = async () => {
      try {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Failed to load previous test answers",
        });
      }
    };
  ...
  // Redirect handled in useEffect or above
  if (!isAuthenticated || !user || !test || !isTestPurchased) {
    return null;
  }
