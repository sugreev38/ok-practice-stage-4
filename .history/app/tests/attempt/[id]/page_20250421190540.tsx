...
export default function AttemptTestPage({ params }: AttemptTestPageProps) {
  ...
  const validateAuth = useCallback(async () => {
    try {
      ...
      // Verify token by fetching user data with retry
      const attemptFetch = async () => {
        ...
        if (!response.ok) {
          let errorMessage = `Failed to verify token: HTTP ${response.status}`;
          try {
      ...
      const response = await attemptFetch().catch(async (error) => {
        if (error.message.includes("Server error")) {
          console.warn("Retrying token verification...");
  ...
  }, [authState.isAuthenticated, authState.token, testId, router, toast]);
  ...
  // Fetch purchased tests when authenticated
  useEffect(() => {
    if (authState.isAuthenticated && !authState.isLoading) {
      fetchPurchasedTests();
    }
  }, [authState.isAuthenticated, authState.isLoading, fetchPurchasedTests]);
  ...
  useEffect(() => {
    if (test && answers.length === 0 && isTestPurchased) {
      ...
      setTimeLeft(Math.max(test.duration || 10, 1) * 60); // Ensure at least 1 minute
  ...
  useEffect(() => {
    if (!test && !isPageLoading && !authState.isLoading) {
      toast({
  ...
  }, [test, isPageLoading, authState.isLoading, router, toast]);

  // Update test status to In Progress
  useEffect(() => {
    const updateTestStatus = async () => {
      if (isTestPurchased && test && authState.token) {
        try {
  ...
  }, [isTestPurchased, testId, test, isPageLoading, authState.token, toast]);
  ...
  useEffect(() => {
    const fetchTestResult = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch test result: HTTP ${response.status}`);
        }
    ...
    if (isTestPurchased && !isPageLoading && authState.isAuthenticated) {
      fetchTestResult();
    }
  }, [isTestPurchased, testId, isPageLoading, authState.isAuthenticated, authState.token, toast]);
  ...
  const submitTest = async () => {
    if (!test || !authState.user || test.questions.length === 0) {
      toast({
        title: "Error",
  ...
  if (authState.isLoading || isPageLoading) {
    return <LoadingSkeleton />;
  }

  // If not authenticated, redirect (handled in validateAuth)
  if (!authState.isAuthenticated || !authState.user) {
    return null;
  }

  // If test not found or not purchased, redirect (handled in useEffect)
