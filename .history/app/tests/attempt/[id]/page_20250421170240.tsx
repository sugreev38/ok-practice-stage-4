  ...
  useEffect(() => {
    const initializeTest = async () => {
      ...
      try {
        ...
        if (!userData?.success) throw new Error("Authentication failed")
        ...
        if (!purchasedData?.success) throw new Error("Failed to fetch purchased tests")
        setPurchasedTests(purchasedData.purchasedTests)

        // Check test purchase status
        const isPurchased = purchasedData.purchasedTests.some((t: any) => t.id === testId)
        if (!isPurchased) {
          toast({
            title: "Test not purchased",
            description: "Please purchase the test to continue",
            variant: "destructive"
          })
          router.push("/tests")
          return
        }
        setIsTestPurchased(true)
    ...
    initializeTest()
  }, [router, testId, toast, test])

  // Rest of the component remains the same...
  // [Keep all other useEffect hooks and component logic unchanged]

  // Loading state render
  if (isPageLoading || !test || !isTestPurchased) {
    return (
