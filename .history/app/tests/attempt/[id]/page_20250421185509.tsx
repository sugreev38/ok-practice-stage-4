...
export default function AttemptTestPage({ params }: AttemptTestPageProps) {
  ...
  const hasRedirectedRef = useRef(false); // Prevent redirect loops
  ...
  if (!params.id || typeof params.id !== "string") {
    if (!hasRedirectedRef.current) {
      ...
      toast({
        ...
        description: "The test ID is missing or invalid.",
        variant: "destructive",
      });
  ...
  // Memoized fetchPurchasedTests to prevent duplicate calls
  const fetchPurchasedTests = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        if (!isPurchased && !hasRedirectedRef.current) {
          hasRedirectedRef.current = true;
          toast({
  ...
  useEffect(() => {
    ...
    // If authenticated, fetch purchased tests
    if (isAuthenticated && user) {
      fetchPurchasedTests();
    }
  }, [isAuthenticated, loading, user, fetchPurchasedTests]);
  ...
  }, [test, answers.length, router, toast]);
  ...
  }, [test, isPageLoading, router, toast]);
  ...
  }, [isTestPurchased, testId, test, isPageLoading, toast]);
  ...
  }, [isTestPurchased, testId, isPageLoading, isAuthenticated, toast]);
