...
export default function AttemptTestPage({ params }: { params: { id: string } }) {
  ...
  const resolvedParams = params; // Fixed: Removed React.use(params)
  ...
  useEffect(() => {
    if (test && answers.length === 0) {
      ...
      setTimeLeft(test.duration * 60 || 3600) // Default to 1 hour if duration not specified
    }
  }, [test, answers.length])
  ...
  }, [timeLeft > 0]); // Fixed: Adjusted dependency to avoid unnecessary re-renders
  ...
  return (
    <div className="flex min-h-screen flex-col">
              ...
              <Card className="mb-6">
                ...
                <CardContent>
                  ...
                  <RadioGroup value={answers[currentQuestion]} onValueChange={handleAnswerSelect}>
                    {test.questions[currentQuestion].options.map((option: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined, index: React.Key | null | undefined) => (
      ...
      {/* Time Up Dialog */}
      <AlertDialog open={isTimeUpDialogOpen} onOpenChange={(open) => open || submitTest()}>
        <AlertDialogContent>
          <AlertDialogHeader>
