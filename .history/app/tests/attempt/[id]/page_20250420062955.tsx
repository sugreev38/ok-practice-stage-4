...
import { useAuthStore, useTestStore } from "@/lib/store"
...
export default function AttemptTestPage({ params }: { params: { id: string } }) {
  ...
  const test = testCatalog.all.find((t) => t.id === test)
  ...
  const submitTest = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/tests/results/${test}`)
      } else {
        toast({
          title: "Error",
  ...
  // Loading state
  if (isPageLoading || isLoading || !test) {
    return (
                ...
                <CardContent>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {test.questions.map((_: any, index: string | number | bigint | ((prevState: number) => number) | null | undefined) => {
