...
// import { Package, Search, ShoppingCart, Building, Shield, BookOpen, FileText, PenToolIcon as Tool, Landmark, Train, Award, ChevronDown, ChevronRight, Clock, Star, Filter, SlidersHorizontal, X, Users, GraduationCap } from 'lucide-react'
// import {
//   Sheet,
//   SheetContent,
//   SheetDescription,
...
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible"
import { useStore, type CartItem, type PurchasedTest } from "@/lib/store"
import { toast } from "@/components/ui/use-toast"
import { testCatalog } from "@/lib/test-catalog"
import {
  Package,
...
export default function TestsPage() {
  ...
  const {
    ...
    cartItems = [],
    purchasedTests = [],
  } = useStore() || { addToCart: () => {}, cartItems: [], purchasedTests: [] }
  ...
  const handleAddToCart = (test: TestType) => {
    // Check if test is already purchased
    if (purchasedTests.some((pt: { id: number }) => pt.id === test.id)) {
      toast({
        title: "Already purchased",
        description: "You already own this test. Check your purchased tests.",
        variant: "destructive",
  ...
  // Add a function to handle attempting a test directly
  const handleAttemptTest = (test: TestType) => {
    // In a real app, you would check if the user has purchased the test
    // For demo purposes, we'll add it to purchased tests
    const purchasedTest: PurchasedTest = {
      id: test.id,
      title: test.title,
      description: test.description,
      price: test.price,
