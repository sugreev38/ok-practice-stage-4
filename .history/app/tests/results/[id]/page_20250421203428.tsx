...
export default function TestResultsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    // If no test result, redirect to purchased tests
    if (!testResult && !isLoading) {
      toast({
        title: "Test result not found",
        description: "No results found for this test.",
        variant: "destructive",
      })
  ...
  }, [testResult, router, isLoading, toast])

  if (isLoading || !test || !testResult) {
    return (
              ...
              <CardContent className="space-y-6">
                <div className="flex flex-col items-center justify-center">
                  <div className="relative flex h-32 w-32 items-center justify-center rounded-full border-8 border-muted">
                    <div className="text-center">
                      <Medal
                        className={`h-8 w-8 mx-auto ${percentile >= 90 ? "text-yellow-500" : percentile >= 70 ? "text-gray-400" : "text-amber-600"}`}
                      />
