...
// Get purchased tests
export async function getPurchasedTests(testIds: any[]) {
  try {
    const auth = await getCurrentUser()
    ...
    const formattedTests = purchasedTests.map((purchase: { test: { id: any; title: any; description: any; price: any; duration: any; numQuestions: any }; status: any; completed: any; purchaseDate: { toISOString: () => any } }) => ({
      id: purchase.test.id,
