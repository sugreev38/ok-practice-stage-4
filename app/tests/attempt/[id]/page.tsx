...
export default function AttemptTestPage({ params }: { params: { id: string } }) {
  ...
  const [timeLeft, setTimeLeft] = useState(3600) // Set to 1 hour (3600 seconds)
  ...
  useEffect(() => {
    if (timeLeft <= 0 && !isTimeUp) {
      ...
      toast({
        title: "Time's Up!",
        description: "The 1-hour time limit has ended. Please submit your test manually.",
        variant: "destructive",
      })
      return
    }
  ...
  // Handle answer selection (disable if time is up)
  const handleAnswerSelect = (value: string) => {
    if (isTimeUp) {
      toast({
        title: "Time's Up",
        description: "You cannot change answers after the time limit.",
        variant: "destructive",
      })
      return
    }
  ...
  const toggleFlagged = () => {
    if (isTimeUp) {
      toast({
        title: "Time's Up",
        description: "You cannot flag questions after the time limit.",
        variant: "destructive",
      })
      return
    }
      ...
      <header className="sticky top-0 z-10 flex h-16 items-center gap-4 border-b bg-background px-6">
        ...
        <div className="ml-auto flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className={`font-medium ${isTimeUp ? "text-red-500" : ""}`}>
            ...
            <div className="md:col-span-3">
              <Card className="mb-6">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      ...
                      <Badge variant="outline" className="mr-2">
                        {test.questions[currentQuestion].difficulty}
                      </Badge>
                      {test.questions[currentQuestion].subcategory && (
                        <Badge variant="outline" className="mr-2">
                          {test.questions[currentQuestion].subcategory}
                        </Badge>
                      )}
                ...
                <CardContent>
                  ...
                  <RadioGroup value={answers[currentQuestion]} onValueChange={handleAnswerSelect} disabled={isTimeUp}>
            ...
            <AlertDialogDescription>
              ...
              {isTimeUp && (
                <div className="mt-2 flex items-center gap-2 text-red-500">
                  <AlertCircle className="h-4 w-4" />
                  <span>The time limit has expired.</span>
                </div>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
