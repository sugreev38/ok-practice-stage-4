...
export default function RegisterPage() {
  ...
  // Update the email verification function to use consistent token naming
  const verifyEmail = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    setError("")
    ...
    try {
      const response = await fetch("/api/verify-email", {
        ...
        body: JSON.stringify({
          email,
          verificationToken: emailToken, // Change token to verificationToken
          userId,
        }),
      })
        ...
        <CardContent>
          ...
          {currentStep === RegistrationStep.EMAIL_VERIFICATION && (
            <form onSubmit={verifyEmail} className="space-y-4">
              ...
              <Button type="button" className="w-full mt-4" variant="outline" onClick={resendEmailCode}>
                Resend Verification Code
              </Button>
            </form>
          )}
